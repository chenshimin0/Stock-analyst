...
def build_prompt_ai_knowledge(concept_name: str, candidates=None) -> str:
    ...
    """
    if not candidates:
        return f"""你是一位资深A股产业研究员。用户向你咨询「{concept_name}」概念板块，请基于你的专业知识推荐相关股票。

## 产业链分析
请先分析「{concept_name}」的产业链结构：
- **上游**：原材料、核心零部件、设备供应商
- **中游**：制造/加工/集成环节
- **下游**：终端应用、客户、消费场景

## 选股标准
在每个环节中，按以下标准筛选最具投资价值的A股股票：
- 沪深主板上市（60/00开头，排除 300/688/8 开头），非 ST
- **必须**与「{concept_name}」有真实业务关联（不是擦边球）
- 市盈率合理或处于低位
- 具有龙头地位（技术/市占率/客户壁垒）
- 近3年持续盈利

## 输出格式
严格按以下 JSON 输出（无 markdown 代码块，纯 JSON）：

{{"picks":[
  {{"code":"600378","name":"昊华科技","reason":"上游氟化工龙头，六氟化钨产能国内第一，直接供应半导体产业"}},
  {{"code":"002812","name":"恩捷股份","reason":"中游锂电池隔膜龙头，市占率超40%，深度绑定宁德时代"}},
  {{"code":"603650","name":"彤程新材","reason":"下游光刻胶国产替代先锋，KrF光刻胶已量产，进入中芯国际供应链"}}
]}}

每条 reason 必须包含：产业链位置 + 龙头证据 + 与「{concept_name}」的具体关联。
如果该概念不存在或根本无法找到 3 只合规股票，返回 {{"error":"原因"}}。
"""
    rows = "\n".join(
