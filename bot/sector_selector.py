...
def build_prompt_ai_knowledge(concept_name: str, candidates=None) -> str:
    ...
    """
    if not candidates:
        return f"""你是一位资深A股产业研究员。用户向你咨询「{concept_name}」概念板块。

## 重要提示
如果找不到与「{concept_name}」**精确匹配**的上市公司（比如概念太细分），请退而求其次，推荐产业链上**最相关**的股票。例如：
- 用户搜"高纯5N氧化镝"→ 没有公司专门做这个 → 推荐稀土行业的龙头（北方稀土、盛和资源、广晟有色等），reason 中说明关联
- 用户搜"固态电池电解质"→ 推荐固态电池相关标的（天赐材料、新宙邦等）
**绝不返回 error，宁可选最接近的，也不要放弃。**

## 选股标准
- 沪深主板上市（60/00开头），排除 300/688/8 开头，非 ST
- 优先行业龙头，有真实业务关联（产业链上下游也算）
- 市盈率合理或处于低位

## 输出格式
严格 JSON（无 markdown 代码块）：
{{"picks":[
  {{"code":"600111","name":"北方稀土","reason":"上游稀土龙头，氧化镝/镝铁合金产能国内第一，全球稀土分离市场份额领先"}},
  {{"code":"600392","name":"盛和资源","reason":"中游稀土分离与冶炼龙头，具备氧化镝全产业链能力"}},
  {{"code":"600259","name":"广晟有色","reason":"广东省稀土集团旗下，中重稀土（含镝元素）开采冶炼龙头"}}
]}}

每条 reason 必须包含：产业链位置（上游/中游/下游）+ 龙头证据 + 与「{concept_name}」的关联。
**必须返回 3 只股票，即使不是精确匹配。**
"""
    rows = "\n".join(
